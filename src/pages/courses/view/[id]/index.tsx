...
import { deleteCourseTeacherById } from 'apiSdk/course-teachers';
...
function CourseViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CourseInterface>(
    ...
    () =>
      getCourseById(id, {
        relations: ['school', 'course_teacher'],
      }),
  );

  const course_teacherHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('course_teacher', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Course Teachers
                    </Text>
                    <NextLink passHref href={`/course-teachers/create?course_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.course_teacher?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/course-teachers/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('course_teacher', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/course-teachers/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('course_teacher', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    course_teacherHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
